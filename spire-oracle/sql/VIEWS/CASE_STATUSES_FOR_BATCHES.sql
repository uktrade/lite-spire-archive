--------------------------------------------------------
--  DDL for View CASE_STATUSES_FOR_BATCHES
--------------------------------------------------------

  CREATE OR REPLACE FORCE EDITIONABLE VIEW "SDBMGR"."CASE_STATUSES_FOR_BATCHES" ("ELA_GRP_ID", "BATCH_ID", "STATUS") AS 
  WITH case_status AS
( SELECT    a.ela_grp_id
          , coalesce(i.batch_id, a.batch_id) batch_id
          , CASE
              ...
              WHEN i.incorporation_flag = 1 AND (i.stakeholders_confirmed = 'false' OR i.stakeholders_confirmed IS NULL) AND (tp.ela_grp_id IS NOT NULL OR ueu.ela_grp_id IS NOT NULL) AND a.case_type IN ('SIEL','SITCL')
                THEN 20
              WHEN a.external_application_id IS NULL AND xed.end_date IS NULL AND a.allow_release_of_batch_id = i.batch_id
                THEN 15
              ELSE 10
            END status_code
  ...
  LEFT JOIN ( SELECT ueu1.*
                     , b.status
                FROM   sdbmgr.ultimate_end_users ueu1
                JOIN   sdbmgr.batches b ON b.id = ueu1.batch_id
              ) ueu ON ueu.ela_grp_id = a.ela_grp_id AND ueu.status_control = 'C' AND (ueu.batch_id = i.batch_id OR ueu.status = 'RELEASED')
  LEFT JOIN ( SELECT tp1.*
                     , b.status
                FROM   sdbmgr.third_parties tp1
                JOIN   sdbmgr.batches b ON b.id = tp1.batch_id
              ) tp ON tp.ela_grp_id = a.ela_grp_id AND tp.status_control = 'C' AND (tp.batch_id = i.batch_id OR tp.status = 'RELEASED')
  ...
  GROUP BY  a.ela_grp_id
          , i.batch_id
          , coalesce(i.batch_id, a.batch_id)
          , a.withheld_status
