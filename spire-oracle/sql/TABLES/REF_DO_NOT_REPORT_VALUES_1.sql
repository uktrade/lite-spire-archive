--------------------------------------------------------
--  DDL for Table REF_DO_NOT_REPORT_VALUES
--------------------------------------------------------

  CREATE TABLE "SDBPUBMGR"."REF_DO_NOT_REPORT_VALUES" 
   (	"DNR_TYPE" VARCHAR2(20 BYTE), 
	"DNR_VALUE" VARCHAR2(256 BYTE), 
	 CONSTRAINT "REF_DO_NOT_REPORT_VALUES_PK" PRIMARY KEY ("DNR_TYPE", "DNR_VALUE") ENABLE
   ) ORGANIZATION INDEX NOCOMPRESS ;
  GRANT SELECT ON "SDBPUBMGR"."REF_DO_NOT_REPORT_VALUES" TO "SDBMGR";
  GRANT INSERT ON "SDBPUBMGR"."REF_DO_NOT_REPORT_VALUES" TO "SDBMGR";
  GRANT DELETE ON "SDBPUBMGR"."REF_DO_NOT_REPORT_VALUES" TO "SDBMGR";
